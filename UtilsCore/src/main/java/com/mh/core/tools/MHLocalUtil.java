 ...
 * 本地信息帮助类
 */
public class MHLocalUtil {
	
	public static String mh_uuid = "";
	...
	public static int getNavigationBarHeight(Context context) {
	    ...
	    MHLogUtil.logD("mh", "Navi height:" + height);
	    return height;
	}
	...
	public static String getLocalMacAddress(Context ctx) {
		MHResConfiguration.getSDKLoginSign(ctx);//初始化一下sign值，为请求网络使用
		MHResConfiguration.getSDKLoginTimestamp(ctx);
		getMHUUid(ctx);
		WifiManager wifi = (WifiManager) ctx.getSystemService(Context.WIFI_SERVICE);
		WifiInfo info = wifi.getConnectionInfo();
		String macTmp = info.getMacAddress();
		if (TextUtils.isEmpty(macTmp) || macTmp.endsWith("00:00:00:00:00:00")) {
			MHLogUtil.logD("mhLog", "getLocalMacAddressFromIp");
	 ...
	 * @date 2015年10月12日
	 */
	public static synchronized String getMHUUid(Context ctx) {
		//MHResConfiguration.getSDKLoginSign(ctx);
		if (!TextUtils.isEmpty(mh_uuid) && mh_uuid.length() > 30) {
			return mh_uuid;
		}
		if (MHStorageUtil.isAccess()) {
			String sdcardPath = Environment.getExternalStorageDirectory().getAbsolutePath();
			String dataPath = sdcardPath + File.separator + "Android" + File.separator + "data" + File.separator;
			
			String mhdataPathDir = dataPath + "mh" + File.separator;
			File dir = new File(mhdataPathDir);
			if (!dir.exists()) {
				if(!dir.mkdirs()){
			...
			String mhdataPath = mhdataPathDir +"mhdata-uuid.txt";
			// String mhdata2Path = dataPath + "data/mhdata2.txt";
			// String mhdata3Path = dataPath + "data/mhdata3.txt";

			try {
				mh_uuid = MHFileUtil.readFile(mhdataPath);
				if (!TextUtils.isEmpty(mh_uuid)) {
					return mh_uuid;
				}
				String uuid = UUID.randomUUID().toString();
				if (MHFileUtil.writeFileData(ctx, mhdataPath, uuid)) {
					mh_uuid = uuid;
				}
			} catch (IOException e) {
	...
	/*    if (TextUtils.isEmpty(ipTmp) || ipTmp.equals("0.0.0.0")) {
	    	MHLogUtil.logD("mhLog", "getLocalIpAddress()");
	...
	*/
	public static boolean isWifiAvailable(Context context) {
		...
		if (mobNetInfoActivity == null || !mobNetInfoActivity.isAvailable()) {
			...
			MHLogUtil.logI("mhLog", "netType:" + netType);
	...
	*/
	...
	public static String getSimOperator(Context context){
		...
		if (MHStringUtil.isEmpty(simOperator)) {
			simOperator = "";
		}
		return simOperator;
	}
	...
	public static String getLocalMacAddressFromIp(Context context) {
		...
		try {
			...
			mac_s = MHCipher.binaryToHexString(mac).trim();
