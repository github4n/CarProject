...
//    final static int VOICE_RESULT = 6;
	private static final String TAG = "mh_UploadHandler";
...
/*	boolean handled() {
	...
	public void onResult(int requestCode,int resultCode, Intent intent) {
		MHLogUtil.logD(TAG, "onActivityResult: requestCode-->" + requestCode +",resultCode-->" + resultCode);
		if (resultCode == Activity.RESULT_CANCELED && !mCaughtActivityNotFoundException) {
			// Couldn't resolve an activity, we are going to try again so skip
			// this result.
			MHLogUtil.logD(TAG, "Activity.RESULT_CANCELED");
	...
	/*	if (result == null && intent == null && resultCode == Activity.RESULT_OK) {
		...
		if(resultCode == Activity.RESULT_OK && intent != null){
			...
			if (requestCode == FILE_SELECTED) {
				MHLogUtil.logD(TAG, "requestCode == FILE_SELECTED");
				if (mUploadMessage != null) {
					...
					MHLogUtil.logD(TAG, "mUploadMessage and mFilePathCallback is null");
				}
//			mHandled = true;
			}
		}
...
//			return;
		...
		MHLogUtil.logD(TAG, "acceptType:" + acceptType + " ,capture:" + capture);
...
/*	private Intent createOpenableIntent(String type) {
	...
	private Intent createCameraIntent() {//拍照
	...
	private Intent createCamcorderIntent() {//摄像头
		return new Intent(MediaStore.ACTION_VIDEO_CAPTURE);
	}

	private Intent createSoundRecorderIntent() {//录音机
